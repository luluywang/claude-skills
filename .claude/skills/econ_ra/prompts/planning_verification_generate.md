...
**User's time budget:** [from full_spec.md] hours

| # | Task | Type | Depends On | Est. Time |
|---|------|------|------------|-----------|
| 1 | Load QCEW data from data/raw/qcew.dta, verify structure: should be county-industry-quarter with ~2M rows for 2010-2019 | code | - | 10 min |
| 2 | Load minimum wage data from data/raw/state_mw.csv, reshape to state-quarter panel | code | - | 10 min |
| 3 | Merge QCEW with MW data on state-quarter, verify merge rate >99% | code | 1, 2 | 15 min |
| 4 | Apply sample restrictions: drop NAICS 92 (government), drop 2020, drop counties with <100 establishments | code | 3 | 10 min |
| 5 | Estimate event study with leads/lags -4 to +4 | analysis | 4 | 20 min |

**Total estimated time:** [sum] minutes ([X.X] hours)

### Time Budget Assessment
- User requested: [X] hours
- Estimated total: [Y] hours
- Status: [FITS / EXCEEDS BY Z hours / UNDER BY Z hours]
...
- Simple data loading/saving: 5-10 min
- Data merging/reshaping: 10-20 min
- Basic estimation (single regression): 10-15 min
- Complex estimation (optimization, bootstrap): 20-40 min
- Table generation: 10-20 min
- Figure generation: 15-25 min
- Writing tasks (interpretations, notes): 15-30 min

The orchestrator will compare total estimated time against the user's time budget and negotiate if there's a significant mismatch.
