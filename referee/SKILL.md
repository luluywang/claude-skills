...
```
/referee grade paper.pdf        # Grade referee concerns: severity + priority + response approach
```

Reads the referee reports (or mock reports already generated by this skill) alongside the paper and produces `output/referee_grades.md`. Use this before starting revisions to get a prioritized action plan.

### Evaluate Complaints Mode

```
/referee evaluate complaints.md paper.pdf   # Evaluate RA methodology complaints: validity + impact + ranking
```

Reads a complaints file (e.g. a list of methodology concerns from an RA review) alongside the paper PDF. Spawns one Haiku subagent per complaint to assess validity and impact, then ranks them and writes `output/top_complaints.md`. Use this to triage a large complaint list before preparing a response.
...
| `grade` | `prompts/tasks/grade_concerns.prompt` | Grade referee concerns: severity, priority, response approach |
| `evaluate` | `prompts/tasks/evaluate_complaints.prompt` | Evaluate RA methodology complaints: spawn Haiku per complaint, rank by impact × validity |
...
| "grade referee points", "grade concerns", `/referee grade` | grade_concerns |
| "evaluate complaints", "rank methodology complaints", `/referee evaluate` | evaluate_complaints |
...
```
/path/to/output/referee_grades.md   # Graded + priority-ordered concern list
```

Evaluate complaints mode output:

```
/path/to/output/top_complaints.md   # Ranked + evaluated complaint list
```
...
- `prompts/tasks/grade_concerns.prompt` - Grade referee concerns by severity and priority
- `prompts/tasks/evaluate_complaints.prompt` - Evaluate and rank RA methodology complaints
