...
CACHE_DIR="$SCRIPT_DIR/cache/$BASE_NAME"
OUTPUT_DIR="$SCRIPT_DIR/output"

# Check for required tools
command -v pdfinfo >/dev/null 2>&1 || { echo -e "${RED}pdfinfo not found. Install with: brew install poppler${NC}"; exit 1; }
command -v pdftotext >/dev/null 2>&1 || { echo -e "${RED}pdftotext not found. Install with: brew install poppler${NC}"; exit 1; }
command -v pdfseparate >/dev/null 2>&1 || { echo -e "${RED}pdfseparate not found. Install with: brew install poppler${NC}"; exit 1; }
...
if command -v python3 >/dev/null 2>&1; then
    if python3 -c "import pdfplumber" 2>/dev/null; then
        python3 "$SCRIPT_DIR/extract_tables.py" \
            --input "$PDF_FILE" \
            --output-dir "$CACHE_DIR/tables"
        echo -e "${GREEN}   Table extraction complete${NC}"
    else
        echo -e "${YELLOW}   pdfplumber not installed. Run: pip install pdfplumber${NC}"
