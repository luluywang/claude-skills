#!/bin/bash
# loop.sh — Actor-critic iteration state for prove-loop.
# Adapted from write-loop/scripts/loop.sh. State + artifacts live under
# prove-loop/current/ (relative to the invoked CWD by default).
#
# Usage:
#   ./loop.sh init [max_iterations]     # Initialize state (defaults: max=5)
#                                       # Takes SOURCE_PATH / GOAL / PERSONA / SCOPE env vars.
#                                       # SOURCE_PATH must be a seed .tex file the
#                                       # orchestrator has prepared (statement + any
#                                       # rambly draft).
...
#   ./loop.sh finalize                  # Copy the last actor text to current/final.tex
#
...
if [ -n "${PROVE_LOOP_WORK_DIR:-}" ]; then
    CURRENT_DIR="$PROVE_LOOP_WORK_DIR"
elif PROJECT_ROOT="$(git rev-parse --show-toplevel 2>/dev/null)"; then
    CURRENT_DIR="$PROJECT_ROOT/prove-loop/current"
else
    CURRENT_DIR="$(pwd)/prove-loop/current"
fi
...
case "${1:-status}" in
    init)
        ...
        if [ -z "$SOURCE_PATH" ] || [ ! -f "$SOURCE_PATH" ]; then
            echo "{\"error\": \"init requires SOURCE_PATH env var pointing to an existing seed file\"}"
            exit 1
        fi
        GOAL_JSON=$(printf "%s" "${GOAL:-unspecified}" | json_escape)
        PERSONA_JSON=$(printf "%s" "${PERSONA:-math-strict referee}" | json_escape)
        SCOPE_JSON=$(printf "%s" "${SCOPE:-whole proof}" | json_escape)
        SRC_JSON=$(printf "%s" "$SOURCE_PATH" | json_escape)
        STARTED=$(date -u +"%Y-%m-%dT%H:%M:%SZ")

        # Seed iter_0 with the source text so the actor can build from it.
        ITER0="$CURRENT_DIR/iter_0"
        mkdir -p "$ITER0"
        cp "$SOURCE_PATH" "$ITER0/text.tex"
...
EOF
    ...
    finalize)
        ...
        ITERATION=$(get_json_value "$STATE_FILE" "iteration")
        LAST_TEXT="$CURRENT_DIR/iter_${ITERATION}/text.tex"
        if [ ! -f "$LAST_TEXT" ]; then
            # Fall back to the highest-numbered iter_*/text.tex that exists
            LAST_TEXT=$(ls -d "$CURRENT_DIR"/iter_*/text.tex 2>/dev/null | sort -V | tail -1)
        fi
        if [ -z "$LAST_TEXT" ] || [ ! -f "$LAST_TEXT" ]; then
            echo '{"error": "No iteration text found to finalize"}'; exit 1
        fi
        cp "$LAST_TEXT" "$CURRENT_DIR/final.tex"
        echo "{\"finalized\": true, \"final\": \"$CURRENT_DIR/final.tex\", \"source_iter\": $ITERATION}"
        ;;
